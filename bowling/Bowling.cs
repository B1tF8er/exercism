...
public class BowlingGame
{
    ...
    public int? Score()
    {
        ...
        for (var i = 1; i <= NumberOfFrames; i++)
        {
            ...
            if (IsStrike(frameIndex))
                Strike(ref score, ref frameIndex, i);
            else if (IsSpare(frameIndex))
                Spare(ref score, ref frameIndex, i);
            else
                Frame(ref score, ref frameIndex);
        }

        return CorrectNumberOfRolls(frameIndex) ? score : (int?)null;
    }

    private void Strike(ref int score, ref int frameIndex, int i)
    {
    ...
    private void Spare(ref int score, ref int frameIndex, int i)
    {
        if (rolls.Count <= frameIndex + 2)
            throw new ArgumentException();
    ...
    private void Frame(ref int score, ref int frameIndex)
    {
    ...
    private bool CorrectNumberOfRolls(int frameIndex) =>
        frameIndex == rolls.Count;

    private bool IsStrike(int frameIndex) =>
        rolls[frameIndex] == MaximumFrameScore;

    private bool IsSpare(int frameIndex) =>
        rolls[frameIndex] + rolls[frameIndex + 1] == MaximumFrameScore;
