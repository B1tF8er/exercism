...
public class GoCounting
{
    ...
    private static Owner[][] ParseBoard(string input) =>
        input
            .Split('\n')
            .Select(row =>
                row.Select(CharToPlayer).ToArray()
            )
            .ToArray();
    ...
    private Owner GetPlayer((int, int) coordinate) =>
        board[coordinate.Item2][coordinate.Item1];

    private bool IsEmpty((int, int) coordinate) =>
        GetPlayer(coordinate) == Owner.None;

    private bool IsTaken((int, int) coordinate) =>
        !IsEmpty(coordinate);
    ...
    {
        var neighborColors = coords
            .SelectMany(TakenNeighborCoordinates)
            .Select(GetPlayer);
    ...
    private HashSet<(int, int)> TerritoryHelper(HashSet<(int, int)> remainder, HashSet<(int, int)> acc)
    {
        ...
        var emptyNeighborsCoordinates = remainder.SelectMany(EmptyNeighborCoordinates); 
        var emptyNeighbors = new HashSet<(int, int)>(emptyNeighborsCoordinates);
    ...
    {
        var invalidRows = coord.Item1 < 0 || coord.Item1 >= Rows;
        var invalidColumns = coord.Item2 < 0 || coord.Item2 >= Cols;
        var invalidCoords = invalidRows || invalidColumns;
    ...
    private static HashSet<T> ToSet<T>(IEnumerable<T> value) =>
        new HashSet<T>(value);
    
    private static HashSet<T> ToSingletonSet<T>(T value) =>
        new HashSet<T> { value };    
}