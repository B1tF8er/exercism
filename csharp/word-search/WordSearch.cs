...
public class WordSearch
{
    ...
    public Dictionary<string, ((int, int), (int, int))?> Search(IEnumerable<string> wordsToSearchFor)
        => wordsToSearchFor
            .ToDictionary(word => word, Search);
    ...
    private char? FindChar((int, int) coordinate)
        => coordinate.Item1 > 0 && coordinate.Item1 <= width && coordinate.Item2 > 0 && coordinate.Item2 <= height
            ? rows[coordinate.Item2 - 1][coordinate.Item1 - 1] as char?
            : null;

    private (int, int)[] Positions()
        => Enumerable.Range(1, width).SelectMany(x =>
