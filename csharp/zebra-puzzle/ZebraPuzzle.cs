...
public static class ZebraPuzzle
{
    ...
    private static IEnumerable<T[]> Permutations<T>(this T[] input) =>
        input.Permutations(input.Length);

    private static IEnumerable<T[]> Permutations<T>(this T[] input, int length) =>
        length == 1
            ? input.Select(t => new[] { t })
            : input.Permutations(length - 1)
                .SelectMany(t =>
                    input.Where(e => !t.Contains(e)),
