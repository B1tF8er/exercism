...
public class Zipper : IEquatable<Zipper>
{
    ...
    public Zipper SetValue(int newValue) =>
        new Zipper(newValue, left, right, crumbs);

    public Zipper SetLeft(BinTree binTree) =>
        new Zipper(value, binTree, right, crumbs);

    public Zipper SetRight(BinTree binTree) =>
        new Zipper(value, left, binTree, crumbs);
    ...
    public Zipper Left()
    {
        ...
        var newCrumbs = new[] { new BinTreeLeftCrumb(value, right) }
            .Concat(crumbs)
            .ToList();
    ...
    public Zipper Right()
    {
        ...
        var newCrumbs = new[] { new BinTreeRightCrumb(value, left) }
            .Concat(crumbs)
            .ToList();
    ...
    public bool Equals(Zipper other) =>
        other == null
            ? false
            : ToTree().Equals(other.ToTree());

    public static Zipper FromTree(BinTree tree) =>
        new Zipper(tree.Value, tree.Left, tree.Right, new List<BinTreeCrumb>());
}