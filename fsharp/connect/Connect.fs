...
let isValidCoordinate board (col, row) =
    row >= 0 && row < rows board && col >= 0 && col < cols board

let hasColor color (board:Board) (col, row) =
    Array2D.get board col row = Some color

let adjacent color board (col, row) = 
    let valid coord =
        isValidCoordinate board coord && hasColor color board coord
...
let isWhiteStop board (_, row) =
    row = rows board - 1

let isBlackStop board (col, _) =
    col = cols board - 1

let whiteStart board =
    [for col in 0..cols board - 1 do if hasColor White board (col, 0) then yield (col, 0)]
    |> Set.ofList

let blackStart board =
    [for row in 0..rows board - 1 do if hasColor Black board (0, row) then yield (0, row)]
    |> Set.ofList

let colorWins color stop start board =
    Set.exists (validPath color stop board Set.empty) (start board)

let whiteWins =
    colorWins White isWhiteStop whiteStart

let blackWins =
    colorWins Black isBlackStop blackStart
