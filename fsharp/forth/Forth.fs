...
let breakBy callback xs =
    (xs|> List.takeWhile (callback >> not), xs |> List.skipWhile (callback >> not))

let empty =
    { 
        sStack = []
        sInput = []
        sMapping = defaultMapping
    }
...
let addItems state items =
    { state with sInput = items @ state.sInput }

let parse text state =
    parseText text
    |> addItems state
...
let unaryStackOp op state =
    ...
    | []    ->
        None
    | x::xs ->
        Some { state with sStack = op x @ xs}

let binaryStackOp op state =
    match state.sStack with
    | x::y::xs ->
        Some { state with sStack = op x y @ xs}
    | _        ->
        None

let toBinaryStackOp op x y =
    [op y x]
...
let applyOp op state =
    match op with
    | Plus  ->
        binaryStackOp (toBinaryStackOp (+)) state
    | Minus ->
        binaryStackOp (toBinaryStackOp (-)) state
    | Mult  ->
        binaryStackOp (toBinaryStackOp (*)) state
    | Div   ->
        divOp state
    | Dup   ->
        unaryStackOp (List.replicate 2) state
    | Drop  ->
        unaryStackOp (fun _ -> []) state
    | Swap  ->
        binaryStackOp (fun x y -> [y; x]) state
    | Over  ->
        binaryStackOp (fun x y -> [y; x; y]) state
    | User terms ->
        addItems state terms
        |> Some
...
let flattenOperations state (operations: Item list) =
    operations
    |> List.collect (flattenOperation state)
...
let rec evalState state =
    ...
    | Some s ->
        ...
        | [] -> state
        | (Value v)::xs ->
            { s with sStack = v::s.sStack; sInput = xs }
            |> Some
            |> evalState
        | (Word w)::xs ->
            match w with
...
let rec eval commands (state: ForthState option) =   
    ...
    | [], _ 
        -> state
        |> Option.map (fun x -> List.rev x.sStack)
    | x::xs, Some y ->
        let updatedState = 
            parse x y 
            |> Some
