...
let updateGuesses guess (state: State) =
    { state with guesses = Set.add guess state.guesses }
...
let mkStartState (word: string) =
    {
        progress = Busy allowedFailures
        maskedWord = String('_', word.Length)
        guesses = Set.empty
    }
...
let makeGuess guess (game: Game) =
    game.MakeGuess(guess)

let statesObservable (game: Game) =
    game.States