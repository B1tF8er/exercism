...
let formatDate (culture: IFormatProvider) (date: DateTime) =
    date.ToString("d", culture)

let formatDescription (description: string) =
    match description.Length <= 25 with
    | true -> description
    | false -> description.[0..21] + "..."

let formatChange (culture: IFormatProvider) (change: float) =
    match change < 0.0 with
    | true -> change.ToString("C", culture)
    | false -> change.ToString("C", culture) + " "
