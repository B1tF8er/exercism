...
let rows (input:string list) =
    input |> List.chunkBySize characterHeight
...
let invalidNumberOfColumns row =
    (row |> String.length) % characterWidth <> 0

let invalidNumberOfRows rows =
    (rows |> List.length) % characterHeight <> 0

let isInvalid input =
    input |> invalidNumberOfRows || input |> List.exists invalidNumberOfColumns
