...
let tuple5ToList =
    function (one, two, three, four, five) -> [one; two; three; four; five]
...
let areSequentialCards (rank1, rank2) =
    rank1 = nextRank rank2

let ranksAreSequential =
    List.pairwise >> List.forall areSequentialCards
...
let (|FourOfAKind|_|) hand =
    ...
    | (rank1, 4)::(rank2, 1)::[] ->
        PokerHand.FourOfAKind (rank1, rank2)
        |> Some 
    | _ ->
        None
...
let (|ThreeOfAKind|_|) hand =
    match ranksWithCount hand with
    | (rank1, 3)::(rank2, 1)::(rank3, 1)::[] ->
        PokerHand.ThreeOfAKind (rank1, rank2, rank3)
        |> Some 
    | _ ->
        None
...
let (|TwoPair|_|) hand =
    match ranksWithCount hand with
    | (rank1, 2)::(rank2, 2)::(rank3, 1)::[] ->
        PokerHand.TwoPair (rank1, rank2, rank3)
        |> Some 
    | _ ->
        None
...
let (|OnePair|_|) hand =
    match ranksWithCount hand with
    | (rank1, 2)::(rank2, 1)::(rank3, 1)::(rank4, 1)::[] ->
        PokerHand.OnePair (rank1, rank2, rank3, rank4)
        |> Some 
    | _ ->
        None
...
let (|FullHouse|_|) hand =
    match ranksWithCount hand with
    | (rank1, 3)::(rank2, 2)::[] ->
        PokerHand.FullHouse (rank1, rank2)
        |> Some 
    | _ ->
        None
...
let bestHands hands =
    let pokerHands =
        hands
        |> List.map (fun hand -> hand, parsePokerHand hand)
    let bestHand =
        pokerHands
        |> List.map snd
        |> List.max 
