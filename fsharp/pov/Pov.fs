...
type Crumb<'a> =
    Crumb of 'a * Graph<'a> list * Graph<'a> list

type Zipper<'a> =
    Graph<'a> * Crumb<'a> list
...
let graphToZipper graph =
    (graph, [])

let crumbValue (Crumb (x, _, _)) =
    x
...
let goDown zipper = 
    match zipper with
    | ({ value = x; children = y::ys }, crumbs) ->
        Some (y, Crumb (x, [], ys)::crumbs)
    | _ ->
        None

let goRight zipper =
    match zipper with
    | (current, Crumb (x, left, r::right)::crumbs) ->
        Some (r, Crumb (x, (left @ [current]), right)::crumbs)
    | _ ->
        None
