...
let private generateKey() = 
    ...
    Array.init keyLength (fun _ -> 
        letters
        |> List.item (random.Next(numberOfLetters))
    ) 
    |> String

let private modulo x y =
    ((x % y) + y) % y

let private charToInt (c:char) =
    (int c) - (int 'a')

let private intToChar (i:int) =
    (char)((int 'a') + (modulo i 26))

let private shiftChar operation key char =
    operation (charToInt char) (charToInt key)
    |> intToChar
